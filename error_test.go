...
func TestAPIErrorUnmarshalJSON(t *testing.T) {
	type testCase struct {
		...
		response  string
		hasError  bool
		checkFunc func(t *testing.T, apiErr giteeai.APIError)
	}
	testCases := []testCase{
		...
		},
		{
			...
			hasError: false,
			checkFunc: func(t *testing.T, apiErr giteeai.APIError) {
				assertAPIErrorInnerError(t, apiErr, &giteeai.InnerError{
					Code: "ResponsibleAIPolicyViolation",
					ContentFilterResults: giteeai.ContentFilterResults{
						Hate: giteeai.Hate{
							Filtered: false,
							Severity: "safe",
						},
						SelfHarm: giteeai.SelfHarm{
							Filtered: false,
							Severity: "safe",
						},
						Sexual: giteeai.Sexual{
							Filtered: true,
							Severity: "medium",
						},
						Violence: giteeai.Violence{
							Filtered: false,
							Severity: "safe",
						},
					},
...
func assertAPIErrorMessage(t *testing.T, apiErr giteeai.APIError, expected string) {
	if apiErr.Message != expected {
		t.Errorf("Unexpected APIError message: %v; expected: %s", apiErr, expected)
	}
}

func assertAPIErrorInnerError(t *testing.T, apiErr giteeai.APIError, expected interface{}) {
	if !reflect.DeepEqual(apiErr.InnerError, expected) {
		t.Errorf("Unexpected APIError InnerError: %v; expected: %v; ", apiErr, expected)
	}
}

func assertAPIErrorCode(t *testing.T, apiErr giteeai.APIError, expected interface{}) {
...
func assertAPIErrorParam(t *testing.T, apiErr giteeai.APIError, expected *string) {
	if apiErr.Param != expected {
		t.Errorf("Unexpected APIError param: %v; expected: %s", apiErr, *expected)
	}
}

func assertAPIErrorType(t *testing.T, apiErr giteeai.APIError, typ string) {
	if apiErr.Type != typ {
...
func TestRequestError(t *testing.T) {
	var err error = &giteeai.RequestError{
		HTTPStatusCode: http.StatusTeapot,
		Err:            errors.New("i am a teapot"),
	}

	var reqErr *giteeai.RequestError
