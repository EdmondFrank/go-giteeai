package giteeai //nolint:testpackage // testing private field
...
import (
	...
	"github.com/edmondfrank/go-giteeai/internal/test"
...
func TestHandleErrorResp(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:        "401 GiteeAI Access Denied",
			httpCode:    http.StatusUnauthorized,
			contentType: "application/json",
			body: bytes.NewReader([]byte(
				`{
...
func TestClientReturnsRequestBuilderErrors(t *testing.T) {
	...
	testCases := []TestCase{
		...
		{"CreateChatCompletion", func() (any, error) {
			return client.CreateChatCompletion(ctx, ChatCompletionRequest{Model: Qwen2_7B_Instruct})
		}},
		{"CreateChatCompletionStream", func() (any, error) {
			return client.CreateChatCompletionStream(ctx, ChatCompletionRequest{Model: Qwen2_7B_Instruct})
		}},
